...
def _limpiar_jsx(raw: str) -> str:
    """
    Extrae el bloque JSX/JS de la respuesta de Claude.
    Elimina backticks y lenguaje del bloque de código si vienen presentes.
    """
    raw = raw.strip()

    # Bloque ```jsx ... ``` o ```javascript ... ``` o ```js ... ```
    match = re.search(r"```(?:jsx?|javascript|tsx?)?\s*([\s\S]*?)```", raw)
    if match:
...
# --- FASE 3: GENERAR COMPONENTE REACT ----------------------------------------

def generar_componente_react(
    datos_extraidos: list[dict[str, Any]],
    formato_visualizacion: str
) -> str:
    """
    Recibe los datos extraídos por /ejecuta y el formato de visualización
    indicado por el usuario, y le pide a Claude que genere un componente
    React JSX funcional que renderice esos datos de forma dinámica.

    El componente:
    - Recibe los datos como prop `data` (array de registros)
    - No depende de ninguna librería externa salvo React
    - Usa estilos inline para ser autónomo
    - Se adapta a la estructura real de los datos recibidos
    """

    prompt = (
        "Eres un experto desarrollador de interfaces React.\n\n"
        "Tu tarea es generar un componente React JSX funcional que visualice "
        "los datos que se te proporcionan, siguiendo exactamente el formato de visualización solicitado.\n\n"
        "REGLAS OBLIGATORIAS:\n"
        "1. El componente debe llamarse `DataVisualizer` y ser un export default.\n"
        "2. Recibe una sola prop llamada `data` que es el array de registros que se te pasa.\n"
        "3. NO uses ninguna librería externa. Solo React (puedes usar hooks como useState).\n"
        "4. Usa únicamente estilos inline (style={{...}}) para que el componente sea completamente autónomo.\n"
        "5. El componente debe adaptarse dinámicamente a los campos presentes en `data`; "
        "NO hardcodees nombres de campos.\n"
        "6. Maneja el caso en que `data` esté vacío o sea null mostrando un mensaje amigable.\n"
        "7. Responde ÚNICAMENTE con el código JSX, sin explicaciones, sin backticks, sin markdown.\n\n"
        f"FORMATO DE VISUALIZACIÓN SOLICITADO:\n{formato_visualizacion}\n\n"
        f"DATOS QUE DEBE RENDERIZAR (úsalos para entender la estructura, no los hardcodees):\n"
        f"{json.dumps(datos_extraidos, indent=2, ensure_ascii=False)}\n\n"
        "Genera el componente React ahora:"
    )
    ...
    return _limpiar_jsx(response.content[0].text)
